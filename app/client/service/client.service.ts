...
export class ClientService {
  ...
  createClient(client: Client): Observable<Client> {
    return this.http.post<Client>(this.mainUrl
  ...
  getClientById(id): Observable<Client> {
    ...
    return this.http.get<Client>(`${this.mainUrl
      }/${id}`).pipe(
        catchError(e => {
  ...
  updateClient(client: Client): Observable<Client> {
    return this.http.put<Client>(`${this.mainUrl
      }/${client.id}`, client, { headers: this.httpHeaders }).pipe(
        catchError(e => {
  ...
  delete(id: number): Observable<Client> {
    return this.http.delete<Client>(`${this.mainUrl
      }/${id}`, { headers: this.httpHeaders }).pipe(
        catchError(e => {
          this.router.navigate(['/clients'])
          console.log(e.error.message)
          Swal.fire(e.error.message, e.error.error, 'error');
  ...
  uploadFile(file: File, id): Observable<HttpEvent<{}>> {
    ...
    const req = new HttpRequest('POST', `${this.mainUrl
      }/files`, formData, {
      reportProgress: true
    })
  ...
  getRegions(): Observable<any> {
    return this.http.get(this.mainUrl.concat('/regions')
    )
  }
}
